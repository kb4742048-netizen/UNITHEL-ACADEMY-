...
export async function fetchMembers(): Promise<Member[]> {
  try {
    const res = await fetch(`${API_BASE}/api/members?_t=${Date.now()}`, { cache: 'no-store' });
    if (res.ok) {
      const data = await res.json();
...
export async function updateAppearance(data: any): Promise<any> {
  try {
    ...
    if (!res.ok) {
      ...
      return { success: false, error: errJson.error || `Server error (${res.status})` };
    }
    ...
    return { success: false, error: err.message || 'Failed to update appearance.' };
  }
}
...
export async function generatePatronInvite(patronType: 'Lord Patron' | 'Patron' = 'Lord Patron'): Promise<any> {
  try {
    ...
    if (!res.ok) {
      ...
      return { success: false, error: err.error || 'Failed to generate invitation link.' };
    }
    return await res.json();
  } catch (err: any) {
...
export async function validatePatronInvite(token: string): Promise<any> {
  ...
  return { valid: false, error: 'Network error validating invite link.' };
}
...
export async function registerPatronViaInvite(token: string, data: { name: string; email: string; phone: string; password: string }): Promise<any> {
  try {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      return { success: false, error: err.error || 'Registration via invitation failed.' };
    }
    return await res.json();
  } catch (err: any) {
